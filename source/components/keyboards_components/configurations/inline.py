from aiogram.types import InlineKeyboardMarkup
from components.keyboards_components.keyboards.inline import keyb_end_delete_mi, keyb_end_delete_u, \
    keyb_empty_user_list, keyb_choose_write_menu_sender, keyb_pass_check_load, keyb_get_empty_list_organizations, \
    keyb_end_delete_org, keyb_get_empty_list_partners, keyb_end_delete_partners, keyb_get_empty_list_banks, \
    keyb_end_delete_banks, keyb_get_empty_list_payment_accounts, keyb_end_delete_payment_accounts

cf_key_end_delete_mi = InlineKeyboardMarkup(inline_keyboard=keyb_end_delete_mi)
cf_key_end_delete_u = InlineKeyboardMarkup(inline_keyboard=keyb_end_delete_u)
cf_keyb_empty_user_list = InlineKeyboardMarkup(inline_keyboard=keyb_empty_user_list)
cf_keyb_choose_write_menu_sender = InlineKeyboardMarkup(inline_keyboard=keyb_choose_write_menu_sender)
cf_keyb_pass_check_load = InlineKeyboardMarkup(inline_keyboard=keyb_pass_check_load)
cf_keyb_get_empty_list_organizations = InlineKeyboardMarkup(inline_keyboard=keyb_get_empty_list_organizations)
cf_keyb_end_delete_org = InlineKeyboardMarkup(inline_keyboard=keyb_end_delete_org)
cf_keyb_get_empty_list_partners = InlineKeyboardMarkup(inline_keyboard=keyb_get_empty_list_partners)
cf_keyb_end_delete_partners = InlineKeyboardMarkup(inline_keyboard=keyb_end_delete_partners)
cf_keyb_get_empty_list_banks = InlineKeyboardMarkup(inline_keyboard=keyb_get_empty_list_banks)
cf_keyb_end_delete_banks = InlineKeyboardMarkup(inline_keyboard=keyb_end_delete_banks)
cf_keyb_get_empty_list_payment_accounts = InlineKeyboardMarkup(inline_keyboard=keyb_get_empty_list_payment_accounts)
cf_keyb_end_delete_payment_accounts = InlineKeyboardMarkup(inline_keyboard=keyb_end_delete_payment_accounts)
